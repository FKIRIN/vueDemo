<template>
   <div style="width:100vw;" ref= 'iframeContain'>
   </div>
</template>
<script>
 export default {
   methods:{
      ...
      let page_width = Math.max(
        this.$refs.iframeContain.scrollWidth,
        this.$refs.iframeContain.clientWidth
      );
      ...
      this.$refs.iframeContain.appendChild(oTemp);
    }
   },
   mounted(){
     this.watermark({watermark_txt: this.$store.getters.username});
   }
 }
</script>
<style>
